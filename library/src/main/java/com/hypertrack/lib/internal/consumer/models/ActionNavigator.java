...
public class ActionNavigator {
    ...
    public ActionNavigator(String actionID, ActionNavigatorCallback callback, Handler mHandler) {
    ...
    public void processTimeAwarePolyline(HTAction action) {
        List<GPXLog> timedLocations = TimeAwarePolylineUtils.getDecodedPolyline(action.getTimeAwarePolyline());
    ...
    private void callAnimation(final GPXLog location, final long timeDuration) {
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                callback.moveToLocationWithTimeInterval(actionID,
    ...
    private void logPointDisplayed(GPXLog gpxLog) {
        ...
        HTLog.a("Point displayed: Action " + actionID + ", Display time " + DateTimeUtility.getCurrentTime()
