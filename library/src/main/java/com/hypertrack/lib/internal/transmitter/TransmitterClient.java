...
public class TransmitterClient implements SmartScheduler.JobScheduledCallback, MQTTMessageArrivedCallback,
    ...
    public void createUser(@NonNull String userName, final HyperTrackCallback callback) {
        try {
            // Check if PublishableKey has been configured or not
            if (!HyperTrackUtils.isPublishableKeyConfigured(mContext)) {
                if (callback != null) {
                    callback.onError(new ErrorResponse(Error.Type.PUBLISHABLE_KEY_NOT_CONFIGURED));
                }
                if (eventCallback != null) {
                    eventCallback.onError(new ErrorResponse(Error.Type.PUBLISHABLE_KEY_NOT_CONFIGURED));
                }
                return;
            }
            ...
            if (!HyperTrackUtils.isInternetConnected(mContext)) {
                if (callback != null) {
                    callback.onError(new ErrorResponse(Error.Type.NETWORK_CONNECTIVITY_ERROR));
                }
                return;
            }
            ...
            HyperTrackPostRequest<HyperTrackUser> postNetworkRequest = new HyperTrackPostRequest<>(
                    HTConstants.CREATE_USER_TAG, mContext, url, HTNetworkClient.HT_NETWORK_CLIENT_HTTP,
                    requestBody, HyperTrackUser.class,
                    new HTNetworkResponse.Listener<HyperTrackUser>(){
                        @Override
                        public void onResponse(HyperTrackUser user) {
    ...
    public void startTracking(boolean startService, HyperTrackCallback callback) {
        try {
            ...
            HTLog.i(TAG, "User Tracking started");
    ...
    public void stopTracking(HyperTrackCallback callback) {
        try {
            ...
            HTLog.i(TAG, "User's Tracking stopped.");
